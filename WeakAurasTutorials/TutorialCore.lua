  ...
  function WeakAuras.PointTutorialToFrame(frame)
    ...
    if(frame.GetRect and frame:GetRect()) then
      ...
      if(l~=pl or b~=pb or w~=pw or h~=ph) then
        ...
        pointingFrame:SetWidth(w);
        pointingFrame:SetHeight(h);
...
function WeakAuras.PlayTutorial(tutData, step, fromPrevious)
  ...
  if(stepData.texture) then
    stepFrame.texture:SetWidth(stepData.texture.width);
    stepFrame.texture:SetHeight(stepData.texture.height);
    ...
    stepFrame.texture:SetWidth(100);
    stepFrame.texture:SetHeight(1);
